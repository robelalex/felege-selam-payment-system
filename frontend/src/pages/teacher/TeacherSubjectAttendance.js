// src/pages/teacher/TeacherSubjectAttendance.js
// Subject-teacher period attendance — "was this student in today's class"
// — distinct from TeacherAttendance's homeroom daily attendance.
import React, { useEffect, useState, useCallback } from 'react';
import { useParams, useSearchParams, useNavigate } from 'react-router-dom';
import { ArrowLeft, Loader, RefreshCw, ListChecks, BarChart3 } from 'lucide-react';
import { getSubjectAttendanceRoster, saveSubjectAttendance, getSubjectAttendanceSummaryRecords, extractError } from '../../services/teacherApi';
...
function TeacherSubjectAttendance() {
  ...
  // ✅ NEW — "Take Attendance" (existing) vs "Summary" (new: present/
  // absent/late/excused day-counts over a range), same gap and same fix
  // as TeacherAttendance.js's homeroom version.
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-primary-700 text-white px-4 sm:px-6 py-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span className="font-semibold truncate">{subjectName} — Grade {grade} {section}</span>
        </div>
        {view === 'entry' && (
          <input
            type="date"
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6">
        ...
        {view === 'summary' ? (
          <SubjectAttendanceSummaryPanel subjectId={subjectId} grade={grade} section={section} />
        ) : (
          <>
...
// rate over a date range, for this one subject/grade/section. Same
// aggregation approach as AdminAttendance.js and TeacherAttendance's
// homeroom summary — fetch the flat day-by-day list, group client-side.
function SubjectAttendanceSummaryPanel({ subjectId, grade, section }) {
  ...
  const fetchSummary = useCallback(async () => {
    ...
    setError('');
    try {
      const response = await getSubjectAttendanceSummaryRecords({ subjectId, grade, section, dateFrom, dateTo });
  ...
  }, [subjectId, grade, section, dateFrom, dateTo]);
...
export default TeacherSubjectAttendance;
